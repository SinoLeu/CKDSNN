...
from utils.utils import apply_adaptive_noise,apply_adaptive_flatten
...
class hierarchical_spiking_transformer(nn.Module):
    ...
    def forward_features(self, x,return_inter=False):
        ...
        if return_inter:
            mid_features.append(x)
        if return_inter:
            return x.flatten(3).mean(3),mid_features
        else:
    ...
    def forward(self, x,return_inter=False):
        ...
        if return_inter:
            x,mid_features = self.forward_features(x,return_inter)
            logits = self.head(x.mean(0))
            return logits,mid_features
        else:
...
#     W = 224
    ...
    # ## QKFormer_10_384 torch.Size([2, 96, 56, 56]) torch.Size([2, 192, 28, 28]) torch.Size([2, 384, 14, 14])
    ...
    # model = QKFormer_10_384(T = 4).cuda()
    # model(x)
